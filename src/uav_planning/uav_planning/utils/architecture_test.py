...
class ArchitectureTestNode(Node):
    ...
    def armed_callback(self, msg):
        ...
        if not hasattr(self, "_armed_logged"):
            self.get_logger().info(
                f"✓ Received armed status from UAV controller: {self.armed_status}"
            )
    ...
    def run_test(self):
        ...
        if self.test_step == 1:
            ...
            self.get_logger().info(
                "Step 1: Waiting for UAV controller to publish status..."
            )

        elif self.test_step == 3:
            if self.received_position and self.received_armed_status:
                self.get_logger().info(
                    "✓ UAV controller is publishing status correctly"
                )
            else:
                self.get_logger().warn(
                    "✗ Not receiving status from UAV controller"
                )
                missing = []
            ...
            self.waypoint_pub.publish(waypoint_msg)
            self.get_logger().info(
                "✓ Published second test waypoint [-2.0, 4.0, 3.5]"
            )
            ...
            self.get_logger().info(
                f"  - Position updates: {'✓ Working' if self.received_position else '✗ Failed'}"
            )
            self.get_logger().info(
                f"  - Armed status: {'✓ Working' if self.received_armed_status else '✗ Failed'}"
            )
            self.get_logger().info("  - Waypoint publishing: ✓ Working")

            if self.received_position and self.received_armed_status:
                self.get_logger().info(
                    "🎉 Separated architecture is working correctly!"
                )
                self.get_logger().info("You can now:")
                self.get_logger().info(
                    "  1. Use the UAV controller for low-level control"
                )
                self.get_logger().info(
                    "  2. Use the path generator action for high-level missions"
                )
                self.get_logger().info(
                    "  3. Test with: ros2 launch uav_planning uav_system_demo.launch.py"
                )
            else:
                self.get_logger().error(
                    "❌ Architecture test failed - check if UAV controller is running"
                )
